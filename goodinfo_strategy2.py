...
def run(target_date=None, quiet=False):
    ...
    log(f"\n{'='*55}")
    log(f"策略2 選股結果（{today}）：共 {len(result)} 檔")
    log(f"EPS 判斷：本益比 > 0，且最近一季 EPS ≥ 去年同期")
    log(f"新增條件：成交量為4日內新高")
    log(f"{'='*55}")
    ...
    if not result.empty:
        ...
        if not quiet:
            ...
            fname     = os.path.join(base_dir, f"策略2_選股_{today}.xlsx")
            fname_csv = os.path.join(base_dir, f"策略2_選股_{today}.csv")
            try:
